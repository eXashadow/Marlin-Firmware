...
uint8_t lcdDrawUpdate = LCDVIEW_CLEAR_CALL_REDRAW; // Set when the LCD needs to draw, decrements after every draw. Set to 2 in LCD routines so the LCD gets at least 1 full redraw (first redraw is partial)
  ...
  static void lcd_control_volumetric_menu();
    ...
    #endif
    //static void lcd_info_thermistors_menu();  //alejanson
    //static void lcd_info_board_menu();        //alejanson
...
void kill_screen(const char* lcd_msg) {
  ...
  static void lcd_main_menu() {
    ...
    MENU_ITEM(submenu, MSG_CONTROL, lcd_control_menu);
    ...
    #if ENABLED(LCD_INFO_MENU)    //alejanson
      ...
      if (card.cardOK) {
        if (card.isFileOpen()) {
          ...
          #if !PIN_EXISTS(SD_DETECT)
            //MENU_ITEM(gcode, MSG_CNG_SDCARD, PSTR("M21"));  // SD-card changed by user   //alejanson
          #endif
        }
      }
      else {
  ...
  static void lcd_control_menu() {
    ...
    //MENU_ITEM(submenu, MSG_VOLUMETRIC, lcd_control_volumetric_menu);    //alejanson
   ...
   */
    ...
    static void lcd_info_printer_menu() {
      ...
      //STATIC_ITEM(SHORT_BUILD_VERSION, true);                          // x.x.x-Branch      //alejanson
      STATIC_ITEM(STRING_DISTRIBUTION_DATE, true);                     // YYYY-MM-DD HH:MM
      STATIC_ITEM(MACHINE_NAME, true);                                 // My3DPrinter
      STATIC_ITEM(WEBSITE_URL, true);                                  // www.my3dprinter.com
      //STATIC_ITEM(MSG_INFO_EXTRUDERS ": " STRINGIFY(EXTRUDERS), false); // Extruders: 2     //alejanson
      END_SCREEN();
    }
    ...
    static void lcd_info_menu() {
      ...
      //MENU_ITEM(submenu, MSG_INFO_BOARD_MENU, lcd_info_board_menu);            // Board Info >    //alejanson
      //MENU_ITEM(submenu, MSG_INFO_THERMISTOR_MENU, lcd_info_thermistors_menu); // Thermistors >   //alejanson
      #if ENABLED(PRINTCOUNTER)
